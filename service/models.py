...
class Shopcart(db.Model):
    ...
    def set_items(self, items_payload: list):
        ...
        self.total_items = int(
            sum(int(getattr(i, "quantity", 0) or 0) for i in getattr(self, "items", []))
        )
    ...
    def list_shopcarts(
        ...
        """
        logger.info(
            "Listing shopcarts (customer_id=%s, status=%s, page=%d, limit=%d)",
            customer_id,
            status,
            page,
            limit,
        ...
        results = (
            query.order_by(cls.created_date.desc())
            .offset((page - 1) * limit)
            .limit(limit)
            .all()
        )
