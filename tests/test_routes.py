...
class TestYourResourceService(TestCase):
    ...
    def test_require_quantity_increment_non_integer(self):
        ...
        with self.assertRaises(ValidationError) as ctx, app.test_request_context():
            _require_quantity_increment_from_payload({"quantity": object()})
    ...
    def test_resolve_price_invalid_value(self):
        ...
        with self.assertRaises(ValidationError) as ctx, app.test_request_context():
            _resolve_price_for_new_item(None, "not-a-number")
        self.assertEqual(ctx.exception.status_code, status.HTTP_400_BAD_REQUEST)
